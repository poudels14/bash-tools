...
export CDD_FILE="$(pwd)/cdd_completion_list"
CDD_MAP="$(pwd)/cdd_completion_map.csv"
PYTHON_SCR="$(pwd)/cdd_get_value.py"
...
_cdd(){
 ...
 # [ ! -z ${AUTOSERV} ] && \
	COMPREPLY=( $( compgen -W "$(cat ${CDD_FILE})" -- $cur ) )
}
